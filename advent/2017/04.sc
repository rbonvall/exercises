...
""".trim.stripMargin.lines.map(_.split("""\s+""").toSeq).toSeq

def areAllUnique[T](ts: Seq[T]) = ts.length == ts.toSet.size

println(input count areAllUnique)
println(input.map(_.map(_.sorted)) count areAllUnique)
